...
void run_cases() {
    ...
    vector<Mint> poly_x(B + 1);
    for(int i=A;i<=B;i++) {
        poly_x[i] = 1;
    }

    vector<Mint> poly_y(C + 1);
    for(int i=B;i<=C;i++) {
        poly_y[i] = 1;
    }

    vector<Mint> poly_all_sum = poly_x * poly_y;

    vector<int64_t> prefix(B + C + 1);

    for(int i=B+C;i>=1;i--) {
        if(i != B + C) {
            prefix[i] = prefix[i + 1];
        }
        prefix[i] += int64_t(poly_all_sum[i]);
    }

    // debug() << imie(poly_all_sum);
    // debug() << imie(prefix);
