...
void run_cases() {
    ...
    vector<int> min_pos(N + 1), max_pos(N + 1);
    iota(all(min_pos), 0);
    iota(all(max_pos), 0);
    ...
    for(int i=1;i<=M;i++) {
        max_pos[A[i]] = max(max_pos[A[i]], tree.query_suffix(pos[A[i]]));
        min_pos[A[i]] = 1;
        tree.set(pos[A[i]], 0);
        pos[A[i]] = i + N;
        tree.set(pos[A[i]], 1);
    ...
    for(int i=1;i<=N;i++) {
        max_pos[i] = max(max_pos[i], tree.query_suffix(pos[i]));
    }

    for(int i=1;i<=N;i++) {
        cout << min_pos[i] << " " << max_pos[i] << nl;
    }
