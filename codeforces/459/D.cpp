...
void run_cases() {
    int n;
    cin >> n;
    fenwick_tree<int64_t> left_tree, right_tree;
    left_tree.init(n + 10); right_tree.init(n + 10);
    ...
    vector<int> prefix(n + 10), suffix(n + 10);
    for(int i=n-1;i>=0;i--) {
        suffix[a[i]]++;
        right_tree.update(suffix[a[i]], 1);
    }

    for(int i=0;i<n-1;i++) {
        prefix[a[i]]++;
        left_tree.update(prefix[a[i]], 1);
        right_tree.update(suffix[a[i]], -1);
        suffix[a[i]]--;
        ans += right_tree.query(0, prefix[a[i]]);
    }
    cout << ans;
}
